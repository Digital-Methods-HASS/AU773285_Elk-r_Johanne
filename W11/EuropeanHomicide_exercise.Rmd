---
title: "Are we more civilized today? Homework W11"
...
```{r eval=TRUE}
names(Western_Europe)[4] <- "homecides_per_100k"
...
```{r plot_homicides, eval=TRUE}
ggplot(data = Western_Europe, aes(x=Year, y=homecides_per_100k, colour = Entity))+
  geom_point()+
  theme_minimal()+
  labs(title="Long Term Trends in Homecide by Contry", color= "Contry")
```

```{r plot_faccet_homicides, eval=TRUE}
ggplot(data = Western_Europe, aes(x=Year, y=homecides_per_100k, colour = Entity))+
  geom_point()+
  facet_wrap(~ Entity)+
  theme_minimal()+
...
```{r line_faccet_homicides, eval=TRUE}
ggplot(data = Western_Europe, aes(x=Year, y=homecides_per_100k, colour = Entity))+
  geom_line()+
  facet_wrap(~ Entity)+
  theme_minimal()+
...
```{r eval=TRUE}
Kongerækken <- read_csv2("data/Kongerækken csv-kopi 2.csv")
Kongerækken <- mutate(Kongerækken, midyear=Kongerækken$`Regering slut`-(Kongerækken$`Regering slut`- Kongerækken$`Regerings start`)/2, duration= Kongerækken$`Regering slut`- Kongerækken$`Regerings start`)
  
ggplot(data=Kongerækken, aes(x=midyear, y=duration))+
  geom_smooth()+
  geom_point()+
...
Based on the data visualizations comparing homicide trends and Danish monarchs' reign durations, we can assess whether society has become more civilized over time.

The homicide data shows a long-term decline in violent deaths per 100,000 people, particularly from the Middle Ages to modern times. In Scandinavia, homicide rates were significantly higher in the medieval period, often exceeding 10 per 100,000, but dropped dramatically in the 19th and 20th centuries. This suggests increasing social stability, stronger legal institutions, and more effective governance.

On the other hand, the reign durations of Danish monarchs reveal an interesting pattern. Early rulers often had shorter reigns, mostly due to violent power struggles, wars, or assassinations. Over time, reign durations became more stable, reflecting a shift from feudal instability to structured governance, absolutism, constitutional monarchy and in result more peaceful transitions of power.

Together, these trends suggest that societies have indeed become more "civilized" over time. The decline in homicides and the increasing stability in leadership point to improvements in law enforcement and governance.
